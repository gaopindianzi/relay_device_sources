...
* 2012-5-7，为了实现多管理，增加下位机接口，实现以下目标：
  //首先是配置
  配置设备的使用密码
  再设备上配置远程主机的IP地址和UDP端口号
  配置设备的内部IP地址和子网掩码、网关、DNS主机地址等
  ...
  上位机可以通过密码对，对设备进行指令控制，加密密码和解密密码是不一样的


  //实现方法，首先是密码问题，使用不对称密码算法，算法太复杂
  //改为对称加密，双方约定一个秘密，一旦密码泄露，设备需要修改密码，不过这个可以轻松实现，通信的内容都经过加密
  //在设置模式下，无需加密，直接传输原文
  //在工作模式下，必须加密，原文无法接受
  //指令定义：主机信息获取和配置命令，MODBUS控制命令，其他扩展命令，共两条新命令，在一个UDP接口发送和接收
  //如果设备在1分钟内没有接收到控制命令，则会20秒钟广播自己的主机信息，然后往指定IP主机发送自己的主机信息，但不包括密码。
  //主机在规定时间内，于设备通信失败，则显示离线，然后显示重新连接主机中。
  //在线的主机，都可以单选此主机，然后右侧会显示控制，控制开或关的按钮
